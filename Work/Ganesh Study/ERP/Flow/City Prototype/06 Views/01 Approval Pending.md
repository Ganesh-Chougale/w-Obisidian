...
```cshtml
@model IEnumerable<ZanvarGroup.Erp.Business.Masters.City>
@{
    int row_num = 1;
}
<div id="content">
                                        ...
                                        <tr>
                                            <td style="text-align:center">
                                                <label>
                                                    ...
                                                    <input id="Code@(row_num)" type="hidden" value="@item.CityCode">
                                                </label>
                                            </td>
                                            <td>
                                                <a href="City/Approve?CityCode=@item.CityCode" data-ajax-update="#mainContent" data-ajax-mode="replace" data-ajax-method="GET" data-ajax="true">
                                                    @item.CityName
                                                </a>
                                            </td>
    ...
    $(document).ready(function () {
        ...
        $('#btnapprove').click(function (e) {
            ...
            if (CodeList.length != 0) {
                var urlName = ' /masters/City/ApproveMultiple';
                $.ajax({
                    async: false,
                    ...
                    success: function (response, status, xhr) {
                        if (response == 0) {
                            var urlName = ' /masters/City/ApprovalPending';
                            $.ajax({
                                url: urlName,
                                type: "GET",
                                success: function (response, status, xhr) {
